...
function Perfil({ usuario }) {
  ...
  const añadirAficionPersonalizada = () => {
    ...
    setPerfil(prev => ({
      ...prev,
      aficiones: [...(prev.aficiones || []), nuevaAficion.trim()]
    }))
    setNuevaAficion('')
  }
  ...
  const eliminarAficion = (aficion) => {
    setPerfil(prev => ({
      ...prev,
      aficiones: prev.aficiones.filter(a => a !== aficion)
    }))
  }

  if (cargando) return (
    <div style={{ padding: '20px' }}>
      <p className="guia-texto">{t('miPerfil.cargando')}</p>
    </div>
  )
    ...
    <div style={{ paddingBottom: '20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div className="perfil-campo">
          ...
          {editando
            ? <input className="input-reto" value={perfil.nombre || ''} onChange={e => setPerfil(p => ({ ...p, nombre: e.target.value }))} placeholder={t('miPerfil.nombre')} />
        ...
        <div className="perfil-campo">
          ...
          {editando
            ? <input className="input-reto" type="date" value={perfil.fecha_nacimiento || ''} onChange={e => setPerfil(p => ({ ...p, fecha_nacimiento: e.target.value }))} />
          ...
          {editando ? (
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {['hombre', 'mujer', 'otro', 'prefiero'].map(s => (
                <button
                  key={s}
                  className={`btn-dias ${perfil.sexo === s ? 'btn-dias-activo' : ''}`}
                  onClick={() => setPerfil(p => ({ ...p, sexo: s }))}
                  style={{ fontSize: '11px' }}
                >
                  {t(`miPerfil.${s}`)}
                </button>
        ...
        <div className="perfil-campo">
          ...
          {editando
            ? <input className="input-reto" value={perfil.ciudad || ''} onChange={e => setPerfil(p => ({ ...p, ciudad: e.target.value }))} placeholder={t('miPerfil.ciudad')} />
        ...
        <div className="perfil-campo">
          ...
          {editando
            ? <input className="input-reto" value={perfil.pais || ''} onChange={e => setPerfil(p => ({ ...p, pais: e.target.value }))} placeholder={t('miPerfil.pais')} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '8px' }}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {aficionesBase.map(a => (
                  <button
                    key={a}
                    className={`btn-opcion ${perfil.aficiones?.includes(a) ? 'seleccionado' : ''}`}
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <input
                  ...
                  value={nuevaAficion}
                  onChange={e => setNuevaAficion(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') añadirAficionPersonalizada() }}
                />
                <button className="btn-añadir" onClick={añadirAficionPersonalizada}>
      ...
      <button
        className="btn-principal"
        style={{ marginTop: '24px', background: '#E24B4A' }}
